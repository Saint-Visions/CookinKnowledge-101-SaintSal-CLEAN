...
import { ROUTE_MAP, RouteConfig } from './RouteMapper';
...
export class RouteAuditor {
  ...
  private auditLink(link: HTMLAnchorElement, index: number): { connected: boolean; issue?: ButtonIssue } {
    ...
    if (href.startsWith('/') || href.includes(window.location.hostname)) {
      ...
      const routeExists = Object.values(ROUTE_MAP).some((route: RouteConfig) => route.path === path);
  ...
  private findMissingRoutes(): string[] {
    ...
    const knownRoutes = Object.values(ROUTE_MAP).map((route: RouteConfig) => route.path);
  ...
  autoFix(): { fixed: number; remaining: number } {
    ...
    document.querySelectorAll('[data-route]').forEach(element => {
      ...
      if (routeConfig && !(element as HTMLElement).onclick) {
...
export function useRouteAuditor() {
  ...
  const getAuditSummary = () => {
    ...
    results.forEach(result => {
      switch (result.routeStatus) {
        case 'healthy':
          summary.healthyPages++;
          break;
        case 'warning':
          summary.warningPages++;
          break;
        case 'error':
          summary.errorPages++;
          break;
